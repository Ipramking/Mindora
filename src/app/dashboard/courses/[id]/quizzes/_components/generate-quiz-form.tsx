...
export function GenerateQuizForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/dashboard/courses/${courseId}/quizzes/${data.id}`);
    } catch (err) {
    ...
    return (
      <div className="rounded-2xl border border-dashed border-zinc-300 p-6 text-center dark:border-zinc-700">
        <p className="text-zinc-500 dark:text-zinc-400">
          Upload and process a material first, then come back here to generate a quiz.
        </p>
      </div>
    );
  }
    ...
    >
      <div className="flex flex-1 flex-col gap-3 sm:flex-row">
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-zinc-700 dark:text-zinc-300">Questions</span>
          <select
            value={numQuestions}
